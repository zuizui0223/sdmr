"""Mechanical Product-A v2.6 promotion gate for continuation v3.

Continuation v3 changes only technical transport/merge handling before pretruth;
the known-truth and six-part empirical scientific decisions remain unchanged.
"""
...
PURPOSE = "product_a_v2_6_continuation_v3_promotion_preoutcome_contract"
...
EXPECTED_EMPIRICAL = {
    "implementation_sha": "5ce106bb955d0912b1c65ff8dd23a61a3e66aee1",
    "frozen_ref": "frozen/product-a-v2-6-continuation-v3-5ce106bb",
    ...
    "source_presealed_required_M_shards": 216,
    "technical_predecessors": [
        {
            "run_id": 32434610154,
            "failure": "cross_run_artifact_listing_ceiling",
            "pretruth_artifacts": 0,
            "sealed_audit_artifacts": 0,
            "sealed_environment_opened": False,
        },
        {
            "run_id": 32442269594,
            "failure": "blank_optional_selection_trace_raised_EmptyDataError",
            "pretruth_artifacts": 0,
            "sealed_audit_artifacts": 0,
            "sealed_environment_opened": False,
        },
    ],
    "transport_and_merge_repairs_only": True,
...
def load_continuation_v3_promotion_contract(path: str | Path) -> dict[str, Any]:
    source = Path(path)
    payload = json.loads(source.read_text(encoding="utf-8"))
    if payload.get("purpose") != PURPOSE:
        raise ValueError("Product-A continuation v3 promotion purpose changed")
    if payload.get("contract_frozen_before_continuation_v3_sealed_outcome") is not True:
        raise ValueError("continuation v3 promotion was not frozen before sealed outcome")
    if payload.get("new_postoutcome_scientific_thresholds") is not False:
        raise ValueError("continuation v3 promotion cannot add post-outcome thresholds")
    if payload.get("known_truth_source") != EXPECTED_KNOWN_TRUTH:
        raise ValueError("known-truth source changed")
    if payload.get("independent_empirical_continuation_v3_source") != EXPECTED_EMPIRICAL:
        raise ValueError("continuation v3 empirical source changed")
    rule = payload.get("promotion_rule", {})
    if rule.get("logic") != "known_truth_supported_and_independent_empirical_continuation_v3_supported":
        raise ValueError("continuation v3 promotion logic changed")
    for key in (
        ...
        if rule.get(key) is not True:
            raise ValueError(f"continuation v3 promotion rule requires {key}=true")
    failure = payload.get("failure_semantics", {})
    if failure.get("threshold_relaxation_after_outcome_forbidden") is not True:
        raise ValueError("threshold relaxation after continuation v3 outcome must remain forbidden")
...
def _single_decision(root: Path) -> pd.Series:
    ...
    if len(frame) != 1:
        raise ValueError("continuation v3 decision artifact must contain exactly one row")
    return frame.iloc[0]


def _empirical_continuation_v3(root: Path) -> tuple[bool, dict[str, Any], pd.Series]:
    contract = _load_contract(root)
    row = _single_decision(root)
    if contract.get("purpose") != EXPECTED_EMPIRICAL["expected_purpose"]:
        raise ValueError("continuation v3 empirical purpose mismatch")
    if str(contract.get("decision")) != str(row["decision"]):
        raise ValueError("continuation v3 contract/CSV decision mismatch")
    if int(contract.get("n_parts", -1)) != 6:
        raise ValueError("continuation v3 empirical decision must contain six parts")
    for key in (
        ...
        if contract.get(key) is not False:
            raise ValueError(f"continuation v3 empirical source requires {key}=false")
    ...
    if supported:
        for column in (
            ...
            if not bool(row[column]):
                raise ValueError(f"supported continuation v3 decision lacks {column}=true")
    return supported, contract, row


def apply_continuation_v3_promotion(
    *,
    ...
    config = load_continuation_v3_promotion_contract(contract_path)
    kt_ok, kt = _a_known_truth(Path(known_truth_dir))
    emp_ok, emp, emp_row = _empirical_continuation_v3(Path(empirical_continuation_dir))
    ...
    pd.DataFrame([{
        ...
        "independent_empirical_continuation_v3_supported": emp_ok,
    ...
    (out / "promoted_product_a_v2_6_protocol.json").write_text(json.dumps({
        ...
        "empirical_continuation_v3_source": EXPECTED_EMPIRICAL,
        "known_truth_decision": str(kt.get("decision")),
        "empirical_decision": str(emp.get("decision")),
        "new_postoutcome_scientific_thresholds": False,
    }, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    result = {
        "purpose": "product_a_v2_6_continuation_v3_promotion_decision",
        "promotion_contract_sha256": config["contract_sha256"],
        "decision": decision,
        "product_a_v2_6_promoted": promoted,
