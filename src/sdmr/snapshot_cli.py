...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Use --taxa for focal records or --kingdom plus --bounds for a target-group subset."
        )
    )
    ...
    bounds = _read_bounds(args.bounds) if args.bounds else None
    ...
    if args.kingdom and not bounds and not species_names:
        parser.error("Kingdom-only extraction requires --bounds to avoid accidentally materializing a huge global subset")
