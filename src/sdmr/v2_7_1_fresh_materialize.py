"""Materialize one fresh Product-A v2.7.1 part without opening sealed environments."""
...
from .v2_7_1_fresh_contract import (
...
def materialize_fresh_part(
    *,
    ...
    receipt = load_fresh_source_receipt(source_receipt_path)
    ...
    if len(expected_taxa) != 12:
        raise ValueError("fresh confirmation requires exactly 12 taxa")
    ...
    if observed_m != tuple(int(x) for x in design["M_km"]):
        raise ValueError("fresh M grid differs from frozen 150/300/500 km design")
    if not grid["m_strategy"].astype(str).eq("buffer").all():
        raise ValueError("fresh M grid must remain buffer based")
    ...
    for row in grid.itertuples(index=False):
        ...
        if eligible != expected_taxa:
            raise ValueError(f"fresh part lost predeclared taxa: missing={sorted(expected_taxa-eligible)} extra={sorted(eligible-expected_taxa)}")
        sha = occurrence_table_fingerprint(prepared.occurrences)
        if occurrence_sha is None:
            occurrence_sha = sha
        elif sha != occurrence_sha:
            raise ValueError("fresh M specification changed the frozen focal occurrence split")
        prepared_by_name[str(row.name)] = prepared

    first = next(iter(prepared_by_name.values()))
    model_occurrences = first.occurrences.loc[first.occurrences[OUTER_ROLE_COL].astype(str).eq(MODEL_ROLE)].reset_index(drop=True)
    sealed_occurrences_raw = first.occurrences.loc[first.occurrences[OUTER_ROLE_COL].astype(str).eq(SEALED_ROLE)].reset_index(drop=True)
    if set(model_occurrences["species"].astype(str)) != expected_taxa or set(sealed_occurrences_raw["species"].astype(str)) != expected_taxa:
        raise ValueError("fresh outer split lost a predeclared taxon")

    model_backgrounds = {}
    sealed_backgrounds_raw = {}
    for name, prepared in prepared_by_name.items():
        model_backgrounds[name] = prepared.background.loc[prepared.background[OUTER_ROLE_COL].astype(str).eq(MODEL_ROLE)].reset_index(drop=True)
        sealed_backgrounds_raw[name] = prepared.background.loc[prepared.background[OUTER_ROLE_COL].astype(str).eq(SEALED_ROLE)].reset_index(drop=True)
    ...
    if len(raster_specs) != 43:
        raise ValueError(f"fresh confirmation expected 43 active CHELSA predictors, found {len(raster_specs)}")
    featured_occurrences, featured_backgrounds, raster_provenance = extract_protocol_grid_rasters(
        model_occurrences, model_backgrounds, raster_specs
    )
    ...
    for name in prepared_by_name:
        spec_dir = out / "M" / name; spec_dir.mkdir(parents=True, exist_ok=True)
    ...
    result = {
        "purpose": "product_a_v2_7_1_fresh_part_model_pool_materialization",
        "seed": int(seed),
        ...
        "old_six_split_parts_reused": False,
        "old_pilot_focal_artifact_reused": False,
    }
...
def main(argv=None) -> int:
    ...
    p.add_argument("--contract", required=True); p.add_argument("--source-receipt", required=True)
    p.add_argument("--focal", required=True); p.add_argument("--target", required=True); p.add_argument("--taxa", required=True)
    p.add_argument("--grid", required=True); p.add_argument("--manifest", required=True); p.add_argument("--output-dir", required=True)
    p.add_argument("--seed", type=int, required=True); p.add_argument("--sealed-fraction", type=float, required=True)
    a = p.parse_args(argv)
    materialize_fresh_part(contract_path=a.contract, source_receipt_path=a.source_receipt, focal_path=a.focal,
        target_path=a.target, taxa_path=a.taxa, grid_path=a.grid, manifest_path=a.manifest,
        output_dir=a.output_dir, seed=a.seed, sealed_fraction=a.sealed_fraction)
    return 0
