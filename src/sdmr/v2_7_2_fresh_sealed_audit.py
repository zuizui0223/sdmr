"""Open v2.7.2 rank-2 sealed environments once, after pretruth and final fits."""
...
PURPOSE = "product_a_v2_7_2_fresh_part_sealed_audit"
...
def _final_workers(root: Path) -> dict[str, Path]:
    ...
    for path in sorted(root.rglob("contract.json")):
        ...
        if c.get("sealed_occurrence_environment_read") is not False:
            raise ValueError("v2.7.2 final-fit artifact crossed sealed barrier")
        if c.get("deterministic_successor") is not True:
            raise ValueError("v2.7.2 sealed audit received non-deterministic final fit")
        taxon = str(c.get("taxon", ""))
        if not taxon or taxon in found:
            raise ValueError("v2.7.2 final-fit taxon missing or duplicated")
        found[taxon] = path.parent
    if len(found) != 12:
        raise ValueError(
            f"v2.7.2 sealed audit requires exactly 12 final-fit artifacts, found {len(found)}"
        )
    return found
...
def _unavailable_output(
    *, out: Path, materialization: dict, pretruth_root: Path, reason: str,
    ...
    raster_provenance: pd.DataFrame | None = None,
) -> dict[str, object]:
    out.mkdir(parents=True, exist_ok=True)
    part_id = (
        f"seed{int(materialization['seed'])}_"
        f"sealed{float(materialization['sealed_fraction']):.2f}"
    )
    ...
    (audit if audit is not None else pd.DataFrame()).to_csv(
        out / "sealed_empirical_audit.csv", index=False
    )
    (role_summary if role_summary is not None else pd.DataFrame()).to_csv(
        out / "sealed_role_summary.csv", index=False
    )
    ...
    (resolution if resolution is not None else pd.DataFrame()).to_csv(
        out / "sealed_chelsa_resolution_ledger.csv", index=False
    )
    (raster_provenance if raster_provenance is not None else pd.DataFrame()).to_csv(
        out / "sealed_raster_provenance.csv", index=False
    )
    result = {
        "purpose": PURPOSE,
        "part_id": part_id,
...
def run_fresh_sealed_audit(
    *, contract_path: str | Path, part_dir: str | Path, pretruth_dir: str | Path,
    ...
    if materialization.get("purpose") != "product_a_v2_7_2_fresh_part_model_pool_materialization":
        raise ValueError("v2.7.2 sealed audit received wrong materialization")
    if materialization.get("sealed_occurrence_raster_values_extracted") is not False:
        raise ValueError("v2.7.2 sealed environments were opened before declared audit")
    ...
    if pretruth.get("purpose") != "product_a_v2_7_2_fresh_part_pretruth_freeze":
        raise ValueError("v2.7.2 sealed audit requires frozen v2.7.2 pretruth")
    if pretruth.get("deterministic_successor") is not True:
        raise ValueError("v2.7.2 sealed audit received non-deterministic pretruth")
    if int(pretruth.get("model_random_state", -1)) != 0 or int(pretruth.get("selection_process_numpy_seed", -1)) != 0:
        raise ValueError("v2.7.2 sealed audit RNG identity changed before opening evidence")
    out = Path(output_dir)
    ...
    for taxon, root in finals.items():
        ...
        if (
            ...
            or c.get("final_models_serialized_before_sealed_audit") is not True
        ):
            return _unavailable_output(
                out=out, materialization=materialization, pretruth_root=pretruth_root,
                reason=f"final_fit_unavailable:{taxon}",
            )
        if int(c.get("model_random_state", -1)) != 0:
            raise ValueError("v2.7.2 sealed audit final-model RNG identity changed")
    ...
    if missing:
        raise ValueError(f"v2.7.2 sealed audit manifest missing frozen predictors: {missing}")
    manifest = manifest.loc[
        manifest["predictor"].astype(str).isin(required_predictors)
    ].copy()
    ...
    if {spec.predictor for spec in raster_specs} != required_predictors:
        raise ValueError(
            "v2.7.2 sealed raster resolution did not cover exactly the frozen predictor union"
        )

    # First authoritative environmental read of rank-2 sealed rows. No model,
    # candidate, threshold, or random-seed choice can change after this point.
    ...
    sealed_occurrences, sealed_backgrounds, raster_provenance = (
        extract_protocol_grid_rasters(
    ...
    try:
        ...
        for taxon in sorted(finals):
            ...
            audit_predictors = tuple(final_contract["audit_predictors"])
            ...
            if sealed_p.empty:
                raise ValueError(f"v2.7.2 sealed audit missing focal occurrences for {taxon}")
            for name in M_NAMES:
                model_bg_all = pd.read_parquet(
                    part / "M" / name / "model_background.parquet"
                )
                ...
                if sealed_bg.empty:
                    raise ValueError(
                        f"v2.7.2 sealed audit missing reference background for {taxon} {name}"
                    )
                rows = frozen.loc[frozen["M"].astype(str).eq(name)]
                if set(rows["role"].astype(str)) != {"ecological", "auc"} or len(rows) != 2:
                    raise ValueError(
                        f"v2.7.2 final representative denominator changed for {taxon} {name}"
                    )
                for row in rows.itertuples(index=False):
                    selected = tuple(
                        x for x in str(row.selected_predictors).split(",") if x
                    )
                    if int(row.model_random_state) != 0:
                        raise ValueError("v2.7.2 serialized model row lost random-state identity")
                    model = joblib.load(root / str(row.model_file))
                    train_p_scores = score_relative_suitability(
                        model, model_occurrences, selected
                    )
                    ...
                    audit_rows.append({
                        ...
                        "presence_rank": presence_rank_score(
                            sealed_p_scores, sealed_b_scores
                        ),
                        ...
                        "aicc_status": (
                            "not_computed_no_valid_likelihood_mapping_for_"
                            "penalized_class_weighted_logit"
                        ),
                        **profile.as_dict(),
                    })
                    model_rows.append({
    ...
    audit = pd.DataFrame(audit_rows)
    if (
        len(audit) != 12 * len(M_NAMES) * 2
        or audit["taxon"].nunique() != 12
        or audit["M"].nunique() != 3
    ):
        return _unavailable_output(
            out=out, materialization=materialization, pretruth_root=pretruth_root,
            reason="sealed_audit_denominator_incomplete", sealed_environment_read=True,
            audit=audit, model_rows=model_rows, resolution=resolution,
            raster_provenance=raster_provenance,
        )
    required_values = audit.loc[:, list(REQUIRED_SEALED_METRICS)].apply(
    ...
    if not np.isfinite(required_values).all():
        return _unavailable_output(
            ...
            sealed_environment_read=True, audit=audit, model_rows=model_rows,
            resolution=resolution, raster_provenance=raster_provenance,
        )
    ...
    eco = role_summary.loc[
        role_summary["role"].astype(str).eq("ecological")
    ].iloc[0]
    auc = role_summary.loc[
        role_summary["role"].astype(str).eq("auc")
    ].iloc[0]
    summary_required = np.asarray([
        float(eco["mean_presence_rank"]),
        float(auc["mean_presence_rank"]),
    ...
    if not np.isfinite(summary_required).all():
        return _unavailable_output(
            ...
            sealed_environment_read=True, audit=audit, role_summary=role_summary,
            model_rows=model_rows, resolution=resolution,
            raster_provenance=raster_provenance,
        )
    ...
    part_summary = pd.DataFrame([{
        ...
        "all_12_taxa": True,
        "all_3_M_specs": True,
        "mean_presence_rank_delta_vs_auc": float(
            eco["mean_presence_rank"] - auc["mean_presence_rank"]
        ),
