"""Development-only evaluator for v8 conditional shared-information knockout."""
...
DEV_CONFIG = ROOT / "configs" / "conditional_shared_knockout_v8_development.json"
...
def _load():
    ...
    if dev.get("purpose") != "conditional_shared_knockout_v8_development_only":
        raise ValueError("wrong v8 development contract")
    if dev.get("development_only") is not True or dev.get("eligible_for_prospective_performance_claim") is not False:
        raise ValueError("v8 denominator must remain development-only")
    seeds = tuple(int(x) for x in dev.get("consumed_seed_denominator", ()))
    if seeds != tuple(range(15001, 15011)) or tuple(v6.get("seeds", ())) != seeds:
        raise ValueError("v8 must use exactly consumed 15001-15010")
...
def fit_family(family: str, output_dir: str | Path):
    ...
    for seed in dev["consumed_seed_denominator"]:
        ...
        p["v8_detected"] = p["status"].astype(str).eq(CONTRIBUTORY)
        p["v5_detected"] = p["v5_status"].astype(str).eq(CONTRIBUTORY)
        process_parts.append(p)
        r=fit.conditional_route_summary.copy(); r.insert(0,"seed",int(seed)); r.insert(0,"family",family); route_parts.append(r)
...
def aggregate(input_dir: str | Path, output_dir: str | Path):
    ...
    if not all(len(x)==n for x in (pfiles,rfiles,afiles,cfiles)):
        raise ValueError("v8 aggregate requires one artifact per family")
    ...
    if observed != expected or len(cases) != 60:
        raise ValueError("v8 aggregate denominator mismatch")
    v8=_metrics(process,"v8_detected"); v5=_metrics(process,"v5_detected")
    exact=[]
    for (family,seed),g in process.groupby(["family","seed"],sort=True):
        truth=set(g.loc[g.expected_true_process.astype(bool),"process"].astype(str)); v8set=set(g.loc[g.v8_detected.astype(bool),"process"].astype(str)); v5set=set(g.loc[g.v5_detected.astype(bool),"process"].astype(str))
        exact.append({"family":family,"seed":int(seed),"v8_exact":v8set==truth,"v5_exact":v5set==truth})
    exact=pd.DataFrame(exact)
    season=process[(process.process.astype(str)=="seasonality") & (~process.expected_true_process.astype(bool))]
    decision={
        "purpose":"conditional_shared_knockout_v8_consumed_development_decision",
        "development_only":True,"eligible_for_prospective_performance_claim":False,
        "n_cases":60,"n_process_cells":int(len(process)),
        "v8_metrics":v8,"v5_same_case_metrics":v5,
        "v8_exact_process_sets":int(exact.v8_exact.sum()),"v5_exact_process_sets":int(exact.v5_exact.sum()),
        "v8_false_seasonality_n":int(season.v8_detected.sum()),"v5_false_seasonality_n":int(season.v5_detected.sum()),
        "all_complete_interventions_preserve_retained_predictors":bool(audit.loc[audit.complete.astype(bool),"retained_predictors_unchanged"].astype(bool).all()),
