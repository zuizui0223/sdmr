"""Freeze one deterministic Product-A v2.7.2 part before sealed environments open."""
...
def _worker_contracts(root: Path) -> tuple[list[dict], dict[str, Path]]:
    ...
    for path in sorted(root.rglob("contract.json")):
        ...
        if payload.get("sealed_occurrence_environment_read") is not False:
            raise ValueError("v2.7.2 pretruth received a worker that opened sealed environments")
        if payload.get("sealed_occurrence_used_for_selection") is not False:
            raise ValueError("v2.7.2 pretruth received sealed-selected evidence")
        if payload.get("deterministic_successor") is not True:
            raise ValueError("v2.7.2 pretruth received a non-deterministic worker")
        taxon = str(payload.get("taxon", ""))
        if not taxon or taxon in roots:
            raise ValueError("v2.7.2 worker taxon missing or duplicated")
        contracts.append(payload)
        roots[taxon] = path.parent
    if len(contracts) != 12 or len(roots) != 12:
        raise ValueError(f"v2.7.2 part requires exactly 12 unique taxon workers, found {len(roots)}")
    if len({int(c["part_seed"]) for c in contracts}) != 1:
        raise ValueError("v2.7.2 pretruth mixed part seeds")
    ...
    process = pd.DataFrame([
        {
            ...
            "n_expected_routes": 8, "n_observed_routes": 0,
            "n_complete_routes": 0, "n_adequate_routes": 0,
        }
        for taxon in taxa for domain in domains
    ])
    process.to_csv(out / "pretruth_process_status.csv", index=False)
    for filename in (
        "pretruth_base_complete_cell_ledger.csv",
        "pretruth_base_complete_candidate_summary.csv",
        "pretruth_base_adequacy_gate.csv",
        "pretruth_ecological_selection.csv",
        "pretruth_auc_selection.csv",
        "pretruth_knockout_route_status.csv",
        "pretruth_fingerprints.csv",
    ):
...
def run_fresh_pretruth(
    *, contract_path: str | Path, worker_root: str | Path, output_dir: str | Path,
    ...
    if unavailable:
        details = "; ".join(
            f"{c['taxon']}:{c.get('unavailable_stage')}:{c.get('unavailable_reason')}"
            for c in unavailable
        )
        return _write_unavailable(out, contract=contract, workers=workers, reason=details)

    if {int(c.get("model_random_state", -1)) for c in workers} != {0}:
        raise ValueError("v2.7.2 pretruth worker model random-state identity changed")
    if {int(c.get("selection_process_numpy_seed", -1)) for c in workers} != {0}:
        raise ValueError("v2.7.2 pretruth worker process RNG identity changed")
    ...
    for taxon in sorted(roots):
        ...
        if base.empty:
            return _write_unavailable(
                out, contract=contract, workers=workers,
                reason=f"{taxon}:base_metrics_empty",
            )
    ...
    if not complete.eligible_candidates:
        return _write_unavailable(
            out, contract=contract, workers=workers,
            reason="no base procedure has complete 12-taxon x 3-M x 4-fold evidence",
        )
    complete_metrics = base.loc[
        base["candidate"].astype(str).isin(complete.eligible_candidates)
    ].copy()
    ...
    try:
        ...
        return _write_unavailable(
            out, contract=contract, workers=workers,
            reason=f"prediction_adequacy:{exc}",
        )
    adequate_candidates = tuple(ecological.eligible_candidates)
    ecological_rep = str(ecological.candidate)
    try:
        auc_rep, auc_summary = _auc_representative(
            complete_metrics, adequate_candidates
        )
    except ValueError as exc:
        return _write_unavailable(
            out, contract=contract, workers=workers,
            reason=f"auc_comparator:{exc}",
        )
    ...
    try:
        ...
        return _write_unavailable(
            out, contract=contract, workers=workers,
            reason=f"process_status:{exc}",
        )
    if len(process_status) != 12 * len(domains):
        return _write_unavailable(
            out, contract=contract, workers=workers,
            reason="process_status_denominator_incomplete",
        )
    ...
    pd.DataFrame(workers).to_csv(out / "pretruth_worker_contracts.csv", index=False)
    fingerprints = {
        "complete_candidate_summary_sha256": _frame_sha256(
            complete.candidate_summary, ["candidate"]
        ),
        "ecological_selection_sha256": _frame_sha256(
            ecological.recovery_selection.summary, ["candidate"]
        ),
        "auc_selection_sha256": _frame_sha256(auc_summary, ["candidate"]),
        "process_status_sha256": _frame_sha256(
            process_status, ["taxon", "process_domain"]
        ),
        "knockout_route_status_sha256": _frame_sha256(
            knockout_routes, ["taxon", "process_domain", "candidate"]
        ),
    }
    pd.DataFrame([
        {"name": key, "sha256": value} for key, value in fingerprints.items()
    ]).to_csv(out / "pretruth_fingerprints.csv", index=False)
    result = {
        "purpose": PURPOSE,
        "available": True,
...
def main(argv=None) -> int:
    ...
    a = p.parse_args(argv)
    run_fresh_pretruth(
        contract_path=a.contract, worker_root=a.worker_root, output_dir=a.output_dir
    )
    return 0
