"""Mechanical promotion gate for Product-A v2.6 presealed continuation.

The continuation exists only to recover from a computational failure that occurred
before pretruth/sealed audit.  This gate introduces no new ecological threshold:
it composes the already frozen Product-A known-truth decision with the unchanged
independent empirical six-part decision produced by one exact frozen continuation
source.
"""
...
PURPOSE = "product_a_v2_6_continuation_promotion_preoutcome_contract"
...
EXPECTED_EMPIRICAL = {
    "implementation_sha": "da421c88717b193a1c1046c4d6920e841a4b7584",
    "frozen_ref": "frozen/product-a-v2-6-presealed-continuation-da421c88",
    "workflow_file": "product-a-v2-6-empirical-presealed-continuation.yml",
    "artifact_name": "product-a-v2-6-independent-empirical-confirmation-decision-continuation",
    ...
    "source_pretruth_artifacts_observed_before_continuation": 0,
    "source_sealed_audit_artifacts_observed_before_continuation": 0,
}
...
def load_continuation_promotion_contract(path: str | Path) -> dict[str, Any]:
    source = Path(path)
    payload = json.loads(source.read_text(encoding="utf-8"))
    if payload.get("purpose") != PURPOSE:
        raise ValueError("Product-A continuation promotion purpose changed")
    if payload.get("contract_frozen_before_continuation_sealed_outcome") is not True:
        raise ValueError("promotion contract was not frozen before continuation sealed outcome")
    if payload.get("new_postoutcome_scientific_thresholds") is not False:
        raise ValueError("continuation promotion cannot add post-outcome thresholds")
    if payload.get("known_truth_source") != EXPECTED_KNOWN_TRUTH:
        raise ValueError("known-truth source changed")
    if payload.get("independent_empirical_continuation_source") != EXPECTED_EMPIRICAL:
        raise ValueError("independent empirical continuation source changed")
    rule = payload.get("promotion_rule", {})
    if rule.get("logic") != "known_truth_supported_and_independent_empirical_continuation_supported":
        raise ValueError("continuation promotion logic changed")
    for key in (
        ...
        if rule.get(key) is not True:
            raise ValueError(f"continuation promotion rule requires {key}=true")
    failure = payload.get("failure_semantics", {})
    if failure.get("threshold_relaxation_after_outcome_forbidden") is not True:
        raise ValueError("threshold relaxation after continuation outcome must remain forbidden")
...
def _single_decision(root: Path) -> pd.Series:
    ...
    if len(frame) != 1:
        raise ValueError("empirical continuation decision artifact must contain exactly one row")
    return frame.iloc[0]


def _empirical_continuation(root: Path) -> tuple[bool, dict[str, Any], pd.Series]:
    contract = _load_contract(root)
    row = _single_decision(root)
    if contract.get("purpose") != EXPECTED_EMPIRICAL["expected_purpose"]:
        raise ValueError("Product-A continuation empirical purpose mismatch")
    if str(contract.get("decision")) != str(row["decision"]):
        raise ValueError("Product-A continuation contract/CSV decision mismatch")
    if int(contract.get("n_parts", -1)) != 6:
        raise ValueError("Product-A continuation empirical decision must contain six parts")
    for key in (
        ...
        if contract.get(key) is not False:
            raise ValueError(f"Product-A continuation empirical source requires {key}=false")
    ...
    if supported:
        for column in (
            ...
            if not bool(row[column]):
                raise ValueError(f"supported continuation empirical decision lacks {column}=true")
    return supported, contract, row


def apply_continuation_promotion(
    *,
    ...
    config = load_continuation_promotion_contract(contract_path)
    kt_ok, kt = _a_known_truth(Path(known_truth_dir))
    emp_ok, emp, emp_row = _empirical_continuation(Path(empirical_continuation_dir))
    ...
    pd.DataFrame([{
        ...
        "independent_empirical_continuation_supported": emp_ok,
    ...
    protocol = {
        ...
        "empirical_continuation_source": EXPECTED_EMPIRICAL,
        "known_truth_decision": str(kt.get("decision")),
        "empirical_decision": str(emp.get("decision")),
        "new_postoutcome_scientific_thresholds": False,
    }
    (out / "promoted_product_a_v2_6_protocol.json").write_text(json.dumps(protocol, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    result = {
