...
def _unavailable_output(
    *, out: Path, materialization: dict, pretruth_root: Path, reason: str,
    ...
    pd.DataFrame().to_csv(out / "sealed_empirical_audit.csv", index=False)
    pd.DataFrame().to_csv(out / "sealed_role_summary.csv", index=False)
    part.to_csv(out / "part_summary.csv", index=False)
    process.to_csv(out / "process_status.csv", index=False)
    pd.DataFrame().to_csv(out / "frozen_models_audited.csv", index=False)
    pd.DataFrame().to_csv(out / "sealed_chelsa_resolution_ledger.csv", index=False)
    pd.DataFrame().to_csv(out / "sealed_raster_provenance.csv", index=False)
    result = {
        "purpose": PURPOSE,
        "part_id": part_id,
        "available": False,
...
def run_fresh_sealed_audit(
    *, contract_path: str | Path, part_dir: str | Path, pretruth_dir: str | Path,
    ...
    if pretruth.get("available") is not True:
        return _unavailable_output(out=out, materialization=materialization, pretruth_root=pretruth_root,
    ...
    raster_specs, resolution = raster_specs_from_chelsa_manifest(manifest, include_availability=("current",), strict=True)
    ...
    # First authoritative environmental read of sealed rows.
    sealed_occurrences_raw = pd.read_parquet(part / "sealed_occurrences_raw.parquet")
    sealed_backgrounds_raw = {name: pd.read_parquet(part / "M" / name / "sealed_background_raw.parquet") for name in M_NAMES}
    ...
    audit_rows, model_rows = [], []
    for taxon in sorted(finals):
        root = finals[taxon]
        frozen = pd.read_csv(root / "frozen_final_models.csv")
        audit_predictors = tuple(json.loads((root / "contract.json").read_text(encoding="utf-8"))["audit_predictors"])
        model_occurrences_all = pd.read_parquet(part / "model_occurrences.parquet")
        model_occurrences = model_occurrences_all.loc[model_occurrences_all["species"].astype(str).eq(taxon)].reset_index(drop=True)
        ...
        for name in M_NAMES:
            ...
            sealed_bg = sealed_backgrounds[name].loc[sealed_backgrounds[name]["species"].astype(str).eq(taxon)].reset_index(drop=True)
            if sealed_bg.empty:
            ...
            for row in rows.itertuples(index=False):
                ...
                reference_scores = score_ecological_suitability(model, model_bg, selected, observation_predictors=())
                profile = empirical_niche_recovery_profile(model_bg, sealed_p, reference_scores, audit_predictors)
                audit_rows.append({
                    "taxon": taxon, "M": name, "role": str(row.role), "candidate": str(row.candidate),
                ...
                model_rows.append({"taxon": taxon, "M": name, "role": str(row.role), "candidate": str(row.candidate),
                    "selected_predictors": str(row.selected_predictors), "audit_predictors": ",".join(audit_predictors), "model_file": str(row.model_file)})

    audit = pd.DataFrame(audit_rows)
    if len(audit) != 12 * len(M_NAMES) * 2 or audit["taxon"].nunique() != 12 or audit["M"].nunique() != 3:
        raise ValueError("fresh sealed audit did not produce complete 12 x 3M x 2 table")
    role_summary = audit.groupby("role", as_index=False).agg(
        mean_presence_rank=("presence_rank", "mean"), mean_continuous_boyce=("continuous_boyce", "mean"),
        mean_or10=("or10", "mean"), **{metric: (metric, "mean") for metric in RECOVERY_DIRECTIONS},
    ...
    auc_dominates_eco = _dominates(auc, eco); eco_dominates_auc = _dominates(eco, auc)
    part_id = f"seed{int(materialization['seed'])}_sealed{float(materialization['sealed_fraction']):.2f}"
    part_summary = pd.DataFrame([{
        "part_id": part_id, "seed": int(materialization["seed"]), "sealed_fraction": float(materialization["sealed_fraction"]),
        "all_12_taxa": True, "all_3_M_specs": True,
        ...
        "part_available": True, "unavailable_reason": None,
    }])
    process_status = pd.read_csv(pretruth_root / "pretruth_process_status.csv"); process_status["part_id"] = part_id
    out.mkdir(parents=True, exist_ok=True)
    audit.to_csv(out / "sealed_empirical_audit.csv", index=False); role_summary.to_csv(out / "sealed_role_summary.csv", index=False)
    part_summary.to_csv(out / "part_summary.csv", index=False); process_status.to_csv(out / "process_status.csv", index=False)
    pd.DataFrame(model_rows).to_csv(out / "frozen_models_audited.csv", index=False)
    resolution.to_csv(out / "sealed_chelsa_resolution_ledger.csv", index=False); raster_provenance.to_csv(out / "sealed_raster_provenance.csv", index=False)
    result = {
        "purpose": PURPOSE, "part_id": part_id, "available": True, "unavailable_reason": None,
        "sealed_occurrence_environment_read": True, "sealed_occurrence_first_read_after_pretruth_freeze": True,
        "sealed_occurrence_used_for_candidate_selection": False, "sealed_occurrence_used_for_process_status": False,
        "candidate_or_threshold_retuning_after_sealed_read": False, "aicc_interpreted_as_niche_recovery_metric": False,
...
def main(argv=None) -> int:
    ...
    p.add_argument("--pretruth-dir", required=True); p.add_argument("--final-fit-root", required=True); p.add_argument("--manifest", required=True)
    p.add_argument("--output-dir", required=True); a = p.parse_args(argv)
    run_fresh_sealed_audit(contract_path=a.contract, part_dir=a.part_dir, pretruth_dir=a.pretruth_dir,
        final_fit_root=a.final_fit_root, manifest_path=a.manifest, output_dir=a.output_dir); return 0
