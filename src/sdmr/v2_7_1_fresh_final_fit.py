...
def _write_unavailable(out: Path, *, taxon: str, taxon_index: int, part_seed: int, reason: str) -> dict[str, object]:
    ...
    *, contract_path: str | Path, part_dir: str | Path, worker_dir: str | Path,
    ...
    if pretruth.get("available") is not True:
        return _write_unavailable(
            out, taxon=taxon, taxon_index=taxon_index, part_seed=int(pretruth["part_seed"]),
    ...
    if worker.get("purpose") != "product_a_v2_7_1_fresh_model_pool_worker" or worker.get("available") is not True:
        return _write_unavailable(
            out, taxon=taxon, taxon_index=taxon_index, part_seed=int(pretruth["part_seed"]),
            reason="model_pool_worker_unavailable_after_available_pretruth",
        )
    ...
    if len(selected) != len(M_NAMES) * 2:
        return _write_unavailable(out, taxon=taxon, taxon_index=taxon_index, part_seed=part_seed,
            reason="fresh final-fit representative denominator incomplete")
