"""Development-only decorrelation contrast diagnostic for v10.
...
selection uses background environments and spatial block labels only. Process
outcomes are evaluated only after those separator blocks are fixed.
"""
...
def _config() -> dict:
    ...
    if cfg.get("competitor_total_candidate_only") is not True:
        raise ValueError("v10 competitor set must remain frozen to total candidates")
    return cfg


def _block_average(evidence: pd.DataFrame, n_specs: int) -> pd.DataFrame:
    if evidence.empty:
        return pd.DataFrame(columns=["block", "complete", "target_rank_loss", "competitor_rank_loss", "target_density_loss", "competitor_density_loss"])
    rows = []
    for block, group in evidence.groupby("block", sort=True):
        complete = group.loc[group["complete"].astype(bool)]
        ok = len(complete) == int(n_specs)
        rows.append({
            "block": int(block),
            "complete": bool(ok),
            "target_rank_loss": float(complete["target_rank_loss"].mean()) if ok else float("nan"),
            "competitor_rank_loss": float(complete["competitor_rank_loss"].mean()) if ok else float("nan"),
            "target_density_loss": float(complete["target_density_loss"].mean()) if ok else float("nan"),
            "competitor_density_loss": float(complete["competitor_density_loss"].mean()) if ok else float("nan"),
        })
...
def fit_family(family: str, output_dir: str | Path) -> dict[str, object]:
    ...
    for seed in cfg["consumed_seed_denominator"]:
        ...
        summary["total_candidate"] = summary["v5_status"].astype(str).eq(CONTRIBUTORY)
        shared = summary.loc[summary["shared_candidate"].astype(bool), "process"].astype(str).tolist()
        total = set(summary.loc[summary["total_candidate"].astype(bool), "process"].astype(str))
        n_pair = 0; n_sep = 0; n_target_favored = 0; n_competitor_favored = 0
        for p in shared:
            competitors = [q for q in processes if q != p and q in total]
            for q in competitors:
                n_pair += 1
                record = {"family": family, "seed": seed, "target_process": p, "competitor_process": q, "status": "insufficient", "n_separating_blocks": 0, "separating_blocks": "", "pair_decision": "incomplete"}
                try:
                    ...
                    if result.separating_blocks:
                        ...
                        for spec in specs:
                            ev = separator_block_process_contrast(
                                model_presence, background, inner.presence_blocks, inner.background_blocks,
                                separator_blocks=result.separating_blocks,
                                target_process=p, competitor_process=q,
                                target_predictors=closures[p], competitor_predictors=closures[q],
                                ecological_predictors=ecological, observation_predictors=observation,
                                model_spec=spec, knockout_degree=int(cfg["degree"]), knockout_ridge_alpha=float(cfg["ridge_alpha"]), density_probability_epsilon=float(learner["density_probability_epsilon"]),
                            )
                            ev.insert(0, "model_label", spec.label); ev.insert(0, "competitor_process", q); ev.insert(0, "target_process", p); ev.insert(0, "seed", seed); ev.insert(0, "family", family)
                            spec_parts.append(ev)
                        raw = pd.concat(spec_parts, ignore_index=True); contrast_parts.append(raw)
                        block_ev = _block_average(raw, len(specs))
                        decision = classify_separator_pair(block_ev, rank_margin=float(cfg["pairwise_rank_margin"]), density_margin=float(cfg["pairwise_density_margin"]), sem_multiplier=float(cfg["pairwise_sem_multiplier"]))
                        record["pair_decision"] = str(decision["state"])
                        if record["pair_decision"] == "target_favored": n_target_favored += 1
                        if record["pair_decision"] == "competitor_favored": n_competitor_favored += 1
                    else:
                        record["status"] = "no_separator"
                except ValueError as exc:
                    record["status"] = "structural_overlap" if "structurally disjoint" in str(exc) else "insufficient"
                rows.append(record)
        case_rows.append({"family": family, "seed": seed, "n_shared_candidates": len(shared), "n_pair_audits": n_pair, "n_pairs_with_separator": n_sep, "n_target_favored_pairs": n_target_favored, "n_competitor_favored_pairs": n_competitor_favored})
    out = Path(output_dir); out.mkdir(parents=True, exist_ok=True)
    pair = pd.DataFrame(rows); cases = pd.DataFrame(case_rows); contrast = pd.concat(contrast_parts, ignore_index=True) if contrast_parts else pd.DataFrame()
    pair.to_csv(out / "pair_audit.csv", index=False); cases.to_csv(out / "case_summary.csv", index=False); contrast.to_csv(out / "pair_contrast_evidence.csv", index=False)
    return {"family": family, "n_cases": int(len(cases)), "n_pair_audits": int(len(pair)), "n_pairs_with_separator": int((pair.get("status", pd.Series(dtype=str)).astype(str) == "separable_in_principle").sum()), "n_target_favored_pairs": int((pair.get("pair_decision", pd.Series(dtype=str)).astype(str) == "target_favored").sum())}


def aggregate(input_dir: str | Path, output_dir: str | Path) -> dict[str, object]:
    root = Path(input_dir)
    pfiles = list(root.rglob("pair_audit.csv")); cfiles = list(root.rglob("case_summary.csv")); efiles = list(root.rglob("pair_contrast_evidence.csv"))
    if len(pfiles) != len(KNOWN_TRUTH_FAMILIES) or len(cfiles) != len(KNOWN_TRUTH_FAMILIES) or len(efiles) != len(KNOWN_TRUTH_FAMILIES):
    ...
    evidence_parts = [pd.read_csv(x) for x in efiles if Path(x).stat().st_size > 1]
    ...
    decision_counts = pairs["pair_decision"].value_counts().to_dict() if len(pairs) else {}
    shared_cells = int(cases["n_shared_candidates"].sum())
    case_with_any_separator = int((cases["n_pairs_with_separator"] > 0).sum())
    result = {
        "purpose": "decorrelation_contrast_v10_consumed_development_decision",
        ...
        "pair_decision_counts": {str(k): int(v) for k, v in decision_counts.items()},
        ...
        "contrast_outcome_evaluated_only_after_separator_selection": True,
        "fresh_known_truth_validation_authorized": False,
        "fresh_empirical_validation_authorized": False
    }
    out = Path(output_dir); out.mkdir(parents=True, exist_ok=True)
    pairs.to_csv(out / "pair_audit.csv", index=False); cases.to_csv(out / "case_summary.csv", index=False); evidence.to_csv(out / "pair_contrast_evidence.csv", index=False)
