...
The repository design contract is immutable and authoritative.  This module
validates that contract and returns an in-memory compatibility view containing
only the aliases needed by the already frozen deterministic v2.7.2 scientific
core.  It never rewrites the design contract on disk.
"""
...
def _compatibility_view(payload: dict[str, Any]) -> dict[str, Any]:
    """Add only v2.7.2-core aliases in memory; never mutate the frozen contract."""
    ...
    fixed["process_domains"] = list(payload["fixed_design"]["process_domains"])
...
def load_v2_8_3_fresh_confirmation_contract(path: str | Path) -> dict[str, Any]:
    ...
    if payload.get("predeclared_before_any_v2_8_3_structural_or_environmental_outcome") is not True:
        raise ValueError("v2.8.3 design was not predeclared before structural/environmental outcomes")

    geometry = payload.get("upstream_geometry_calibration", {})
    if float(geometry.get("selected_global_sealed_fraction", -1)) != 0.25:
        raise ValueError("v2.8.3 geometry-selected fraction changed")
    if geometry.get("fraction_retuning_allowed") is not False or geometry.get("geometry_result_is_ecological_support") is not False:
        raise ValueError("v2.8.3 geometry/science boundary changed")
    ...
    if int(upstream.get("workflow_run_id", -1)) != EXPECTED_SOURCE_RUN or upstream.get("workflow_conclusion") != "success":
    ...
    if panel.get("blob_sha") != EXPECTED_PANEL_BLOB or panel.get("sha256") != EXPECTED_PANEL_SHA256:
    ...
    if set(pd.to_numeric(frame["candidate_rank"]).astype(int)) != {1}:
        raise ValueError("v2.8.3 selected candidate rank changed")
    ...
    for key in (
        ...
        if inherited.get(key) is not False:
            raise ValueError(f"v2.8.3 inherited scientific semantics changed: {key}")
    ...
    if int(fixed.get("n_confirmation_parts", -1)) != 3:
        raise ValueError("v2.8.3 must contain exactly three nonduplicated parts")
    if (int(fixed.get("outer_folds", -1)), int(fixed.get("spatial_microblocks", -1)), int(fixed.get("assignment_attempts", -1))) != (4, 12, 32):
        raise ValueError("v2.8.3 partition dimensions changed")
    if str(fixed.get("assignment_seed_formula")) != "part_seed + taxon_index*100 + 271":
    ...
    if fixed.get("process_registry_blob_sha") != EXPECTED_PROCESS_REGISTRY_BLOB or fixed.get("process_registry_sha256") != EXPECTED_PROCESS_REGISTRY_SHA256:
        raise ValueError("v2.8.3 process registry pin changed")
    ...
    if (int(library.get("inner_folds", -1)), int(library.get("outer_folds", -1)), int(library.get("max_predictors", -1))) != (3, 4, 8):
    ...
    if tuple(library.get("observation_predictors", ())) != ():
        raise ValueError("v2.8.3 observation-predictor set changed")
    adequacy = fixed.get("prediction_adequacy", {})
    if adequacy != {"chance_auc": 0.5, "minimum_auc_margin": 0.01, "auc_sem_multiplier": 1.0, "complete_outer_fold_evidence_required": True}:
        raise ValueError("v2.8.3 prediction adequacy changed")

    audit = fixed.get("partition_aware_audit_space", {})
    for key, expected in (
        ("minimum_predictor_coverage", 0.95), ("minimum_joint_coverage", 0.8),
        ("minimum_processes", 4), ("minimum_complete_fit_background_rows_per_M_fold", 5),
    ...
    if structural.get("taxon_M_or_seed_replacement_after_structural_result_allowed") is not False or structural.get("incomplete_part_partial_repair_allowed") is not False:
        raise ValueError("v2.8.3 structural result can change the denominator")

    translation = payload.get("single_fraction_denominator_translation", {})
    if (int(translation.get("successor_parts", -1)), int(translation.get("ecological_nondomination_successor_minimum_parts", -1)), int(translation.get("strict_improvement_successor_minimum_parts", -1))) != (3, 2, 2):
        raise ValueError("v2.8.3 denominator translation changed")
    if translation.get("duplicate_0_25_parts_allowed") is not False or translation.get("new_post_calibration_split_seeds_allowed") is not False:
        raise ValueError("v2.8.3 three-part denominator can be manufactured post hoc")
    ...
    if int(eco.get("minimum_parts", -1)) != 2 or int(eco.get("strict_improvement_minimum_parts", -1)) != 2:
        raise ValueError("v2.8.3 ecological part threshold changed")
    ...
    for key in ("post_outcome_candidate_reselection_allowed", "post_outcome_threshold_tuning_allowed", "random_seed_changes_after_outcome_allowed", "scientific_promotion_allowed_by_this_decision"):
    ...
    if boundary.get("execution_allowed") is not False or boundary.get("separate_external_one_shot_authorization_required") is not True:
        raise ValueError("v2.8.3 design contract itself must remain non-executing")
    if boundary.get("scientific_promotion_allowed") is not False or boundary.get("product_b_unblocked") is not False:
        raise ValueError("v2.8.3 design contract crossed promotion boundary")
    return _compatibility_view(payload)


def load_v2_8_3_source_receipt(path: str | Path, *, source_gate_path: str | Path | None = None) -> dict[str, Any]:
...
def v2_7_3_structural_core_view(contract: dict[str, Any]) -> dict[str, Any]:
    ...
    view["inherited_evidence_balanced_partition"] = deepcopy(contract["evidence_balanced_partition"])
    return view
