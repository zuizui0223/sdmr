"""v8 conditional shared-information matched-route process challenge.

For target process P, v8 leaves every non-P predictor untouched and replaces the
P closure by its background-fitted conditional expectation given all other
declared process closures. The matched route therefore removes P-specific
information while retaining environmental structure shared with other processes.

Scientific rank/density evidence thresholds and the v5 state space are inherited
unchanged. Structural overlap between P and another process closure fails closed.
"""
...
class ConditionalSharedKnockoutProcessChallengeFit:
    v5_fit: IntervalEvidenceProcessChallengeFit
    conditional_fold_evidence: pd.DataFrame
    conditional_route_summary: pd.DataFrame
...
def _conditional_route_cv(
    ...
    *,
    ...
    route = f"conditional_shared_knockout::{model_spec.label}::{process}"
    rows: list[dict[str, object]] = []
    audits: list[dict[str, object]] = []
    retained = tuple(x for x in ecological_predictors if x not in set(process_predictors))
    for fold, ((p_train_idx, b_train_idx, p_test_idx, b_test_idx), correction) in enumerate(zip(folds, corrections, strict=True)):
        row = {
            "route": route,
            "route_type": "conditional_shared_information_knockout",
        ...
        audit = {
            ...
            "fold": int(fold),
            "complete": False,
            "retained_predictors_unchanged": False,
            "knockout_fit_background_rows": 0,
        }
        try:
            ...
            p_train_k = knockout.transform(p_train)
            b_train_k = knockout.transform(b_train)
            p_test_k = knockout.transform(p_test)
            b_test_k = knockout.transform(b_test)
            unchanged = all(
                verify_retained_predictors_unchanged(before, after, retained_predictors=retained)
                for before, after in ((p_train, p_train_k), (b_train, b_train_k), (p_test, p_test_k), (b_test, b_test_k))
            )
            if not unchanged:
                raise ValueError("conditional knockout altered retained ecological predictors")
            ...
            if not all(np.isfinite(float(x)) for x in values):
                raise ValueError("conditional route produced non-finite evidence")
            row.update(
                {
                    "complete": True,
            ...
            audit.update(
                {
                    "complete": True,
                    "retained_predictors_unchanged": True,
                    "knockout_fit_background_rows": int(knockout.n_fit_rows),
                }
            )
...
def fit_conditional_shared_knockout_process_challenge(
    presence: pd.DataFrame,
    background: pd.DataFrame,
    ...
    *,
...
) -> ConditionalSharedKnockoutProcessChallengeFit:
        ...
        for model_label in expected_labels:
            ...
            if structural_overlap or not conditioning:
                fold_frame = pd.DataFrame(
                    [
                        {
                            "route": f"conditional_shared_knockout::{model_label}::{process}",
                            "route_type": "conditional_shared_information_knockout",
                ...
                audit = pd.DataFrame(
                    [{"process": process, "model_label": model_label, "fold": i, "complete": False, "retained_predictors_unchanged": False, "knockout_fit_background_rows": 0} for i in range(int(n_splits))]
                )
            else:
                fold_frame, audit = _conditional_route_cv(
                    presence,
                    background,
                    process=process,
    ...
    receipt_payload = "\n".join(
        [
            ...
            "intervention=replace_target_closure_with_background_conditional_expectation_given_other_process_closures",
            f"knockout_degree={int(knockout_degree)}",
            f"knockout_ridge_alpha={float(knockout_ridge_alpha):.12g}",
            "retained_predictors_must_be_numerically_unchanged",
            "process_status=" + process_summary[["process", "status"]].to_csv(index=False),
            "closures=" + closure_summary.to_csv(index=False),
        ]
    )
    return ConditionalSharedKnockoutProcessChallengeFit(
        v5_fit=v5,
        conditional_fold_evidence=folds_out,
        conditional_route_summary=routes,
