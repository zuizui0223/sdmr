"""M-sharded, sealed-blind v2.7.1 model-pool worker.

This is a transport-only continuation of ``v2_7_1_fresh_model_pool_worker``:
the evidence-balanced partition, audit-space choice, procedure library and
process-knockout semantics are identical, but only one frozen M is benchmarked
per process so the three M values can execute independently.
"""
...
def run_fresh_model_pool_M_shard(
    *, contract_path: str | Path, process_registry_path: str | Path,
    ...
    if str(M_name) not in M_NAMES:
        raise ValueError(f"fresh M shard must be one of {M_NAMES}")
    ...
    if materialization.get("purpose") != "product_a_v2_7_1_fresh_part_model_pool_materialization":
        raise ValueError("fresh M shard received the wrong materialization")
    if (
        ...
        raise ValueError("fresh M shard received opened sealed environments")
    if int(materialization.get("seed", -1)) != int(part_seed):
        raise ValueError("fresh M shard seed differs from materialization")
...
def main(argv=None) -> int:
    ...
    p.add_argument("--M", dest="M_name", required=True)
