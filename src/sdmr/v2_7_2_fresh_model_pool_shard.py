"""Primary deterministic M-shard worker for Product-A v2.7.2 empirical confirmation.

Unlike v2.7.1, M sharding is part of the presealed primary v2.7.2 runtime design,
not a post-hoc transport rescue. Every taxon×part×M cell is evaluated in its own
process under the frozen estimator and process RNG identities.
"""
...
def _write_unavailable(
    out: Path, *, taxon: str, taxon_index: int, part_seed: int, M_name: str,
    stage: str, error: str,
) -> dict[str, object]:
    out.mkdir(parents=True, exist_ok=True)
    for name in ("base_fold_metrics.csv", "knockout_fold_metrics.csv", "selection_trace.csv"):
        pd.DataFrame().to_csv(out / name, index=False)
    pd.DataFrame([{
        "taxon": taxon, "M": M_name, "group": "shard", "status": "unavailable",
        "error": error,
...
def run_fresh_model_pool_M_shard(
    *, contract_path: str | Path, process_registry_path: str | Path,
    part_dir: str | Path, taxon: str, taxon_index: int, part_seed: int,
    M_name: str, output_dir: str | Path,
) -> dict[str, object]:
    ...
    if int(part_seed) not in {int(x) for x in contract["fixed_design"]["split_seeds"]}:
        raise ValueError("v2.7.2 part seed is not frozen")
    if not 0 <= int(taxon_index) < 12:
        raise ValueError("v2.7.2 taxon_index must be 0..11")
    if str(M_name) not in M_NAMES:
        raise ValueError(f"v2.7.2 M shard must be one of {M_NAMES}")

    library_cfg = contract["fixed_design"]["procedure_library"]
    process_seed = int(library_cfg["selection_process_numpy_seed"])
    ...
    if materialization.get("purpose") != "product_a_v2_7_2_fresh_part_model_pool_materialization":
        raise ValueError("v2.7.2 M shard received the wrong materialization")
    if (
        materialization.get("sealed_occurrence_raster_values_extracted") is not False
        or materialization.get("sealed_background_raster_values_extracted") is not False
    ):
        raise ValueError("v2.7.2 M shard received opened sealed environments")
    if int(materialization.get("seed", -1)) != int(part_seed):
        raise ValueError("v2.7.2 M shard seed differs from materialization")
    ...
    if occurrence.empty:
        raise ValueError(f"v2.7.2 model-pool occurrence data missing taxon: {taxon}")
    ...
    for name in M_NAMES:
        ...
        if frame.empty:
            raise ValueError(f"v2.7.2 model-pool background missing {taxon} in {name}")
    ...
    if not ecological_predictors:
        return _write_unavailable(
            ...
            M_name=M_name, stage="predictor_admissibility",
    ...
    try:
        ...
        return _write_unavailable(
            out, taxon=taxon, taxon_index=taxon_index, part_seed=part_seed,
            M_name=M_name, stage="structural_partition", error=str(exc),
        )
    ...
    try:
        ...
        return _write_unavailable(
            out, taxon=taxon, taxon_index=taxon_index, part_seed=part_seed,
            M_name=M_name, stage="audit_space", error=str(exc),
        )
    ...
    domain_predictors = {
        str(domain): tuple(
            registry.loc[
                registry["empirical_process_domain"].astype(str).eq(str(domain)),
                "predictor",
            ].astype(str)
        )
    ...
    background = backgrounds[str(M_name)]
    common = dict(
        presence=occurrence,
        background=background,
        presence_groups=partition.presence_folds,
        background_groups=partition.background_folds[str(M_name)],
    ...
    try:
        base = benchmark_recovery_procedures(
            ecological_predictors=ecological_predictors, **common
        )
    except (ValueError, KeyError, np.linalg.LinAlgError) as exc:
        status_rows.append({
            "taxon": taxon, "M": M_name, "group": "base", "status": "failed",
        ...
        status_rows.append({
            "taxon": taxon, "M": M_name, "group": "base", "status": "success",
    ...
    for domain in contract["fixed_design"]["process_domains"]:
        ...
        if not retained:
            status_rows.append({
                "taxon": taxon, "M": M_name, "group": str(domain),
        ...
        try:
            result = benchmark_recovery_procedures(
                ecological_predictors=retained, **common
            )
        except (ValueError, KeyError, np.linalg.LinAlgError) as exc:
            status_rows.append({
                "taxon": taxon, "M": M_name, "group": str(domain),
        ...
        metrics["candidate"] = (
            metrics["base_candidate"].astype(str) + "::exclude::" + str(domain)
        )
        ...
        metrics["excluded_predictors"] = ",".join(
            sorted(excluded & set(ecological_predictors))
        )
        metrics["partition_seed"] = partition_seed
        ...
        if not result.selection_trace.empty:
            ...
            trace["taxon"] = taxon
            trace["M"] = M_name
            trace["group"] = str(domain)
            trace_frames.append(trace)
        status_rows.append({
            "taxon": taxon, "M": M_name, "group": str(domain),
            "status": "success", "error": None, "partition_seed": partition_seed,
        })

    out.mkdir(parents=True, exist_ok=True)
    (pd.concat(base_frames, ignore_index=True) if base_frames else pd.DataFrame()).to_csv(
        out / "base_fold_metrics.csv", index=False
    )
    (
        pd.concat(knockout_frames, ignore_index=True)
        if knockout_frames else pd.DataFrame()
    ...
    result = {
        ...
        "selection_process_numpy_seed": process_seed,
    }
    (out / "contract.json").write_text(
        json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
...
def main(argv=None) -> int:
    ...
    p.add_argument("--process-registry", required=True)
