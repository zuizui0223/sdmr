...
**Predictive success does not identify ecological necessity. Ecological interpretation must separate process necessity, process-information stability and unresolved evidence rather than infer all three from one winning model.**
...
## Branch A — Necessity is a falsification problem
...
Interpretation: exclusion-based inference controlled false necessity and retained true processes, but the safe identified set remained broad. This is a **necessity-safety result**, not the later 0.9889/0.9833 stable-core result.

## Branch B — Process information can be stable without unique model identity

The stability branch is the v2.7.1–v2.7.2 consensus-first sequence and addresses a different estimand.
...
Interpretation: process-level information can remain highly stable even when exact fitted-model identity does not. This is a **process-stability result**, not a 0.99-precision necessity estimate.
...
`fit adequate alternatives -> falsify process necessity -> quantify process stability -> retain contested / unresolved claims`

The central contribution is therefore not a universally superior SDM tuner. It is an inferential framework that separates:

1. **predictive adequacy** — can the model recover withheld observations?
2. **process necessity** — does an adequate explanation survive exclusion of the declared process information?
3. **process stability** — does process information persist across defensible ecological selectors even when fitted models differ?
4. **identification failure** — when evidence is insufficient or selection objectives collapse to the same empirical model, which claims must remain unresolved?
