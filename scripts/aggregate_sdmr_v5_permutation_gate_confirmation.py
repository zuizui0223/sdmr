#!/usr/bin/env python3
"""Aggregate SDMR v5 permutation-gate independent confirmation evidence."""
...
def main() -> None:
    ...
    if len(manifests)!=int(args.expected_shards):
        raise ValueError(f"expected {args.expected_shards} confirmation shards, found {len(manifests)}")
    for path in manifests:
        item=_load(path)
        if item.get("status")!="development_confirmation_evidence":
            raise ValueError("unexpected v5 confirmation shard status")
        if item.get("config_sha256")!=_sha256(config_path):
            raise ValueError("v5 confirmation config hash drift")
        if item.get("activation_sha256")!=_sha256(activation_path):
            raise ValueError("v5 confirmation activation hash drift")
    ...
    if len(summary_files)!=expected or len(fold_files)!=expected:
        raise ValueError("incomplete v5 confirmation outputs")
    ...
    if set(summary["world"].astype(str))!=expected_worlds:
        raise ValueError("v5 confirmation world denominator drift")
    if set(summary["seed"].astype(int))!=expected_seeds:
        raise ValueError("v5 confirmation seed denominator drift")
    if summary.duplicated(["world","seed"]).any():
        raise ValueError("duplicate v5 confirmation cells")
    if len(summary)!=len(expected_worlds)*len(expected_seeds):
        raise ValueError("unexpected v5 confirmation row count")
