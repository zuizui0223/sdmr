#!/usr/bin/env python3
"""Aggregate independently rebuilt Product-A stability parts.

Each part is a complete leakage-safe Product-A grid built from its own
seed/sealed-fraction split, so M/background is regenerated from that split's
model pool before evaluation.  This script only combines already-frozen part
outputs; it never reselects or reruns models.
"""
...
def aggregate(parts_root: Path, output_dir: Path) -> None:
    part_dirs = sorted({p.parent for p in parts_root.rglob("part_metadata.json")})
    if not part_dirs:
    ...
    for run_id, part in enumerate(part_dirs):
        ...
        if expected_occurrence_sha is None:
            ...
            raise ValueError(
                ...
                "Outer roles may change, but the thinned occurrence evidence and CHELSA values must not."
            )
...
def main() -> int:
    ...
    aggregate(Path(args.parts_root), Path(args.output_dir))
    return 0
