#!/usr/bin/env python3
"""Run one blocked SDMR v5 prospective known-truth shard."""
...
def _validate_activation(
    ...
    if scientific.get("program")!="sdmr-v5-prospective-known-truth-v1":
        raise ValueError("wrong v5 prospective scientific contract")
    if execution.get("program")!="sdmr-v5-prospective-known-truth-v1-execution":
        raise ValueError("wrong v5 prospective execution profile")
    if activation.get("purpose")!="execute_sdmr_v5_prospective_known_truth_v1":
        raise ValueError("wrong v5 prospective activation")
    if activation.get("single_activation") is not True:
        raise ValueError("prospective activation must be single-use")
    if activation.get("scientific_contract_blob_sha")!=_git_blob_sha(scientific_path):
        raise ValueError("prospective scientific contract blob SHA mismatch")
    if activation.get("execution_profile_blob_sha")!=_git_blob_sha(execution_path):
        raise ValueError("prospective execution profile blob SHA mismatch")

    for key in ("validation","confirmation","integration"):
        receipt=activation.get(f"{key}_prerequisite",{})
        if receipt.get("terminal_passed") is not True:
            raise ValueError(f"{key} prerequisite did not pass")
        if not str(receipt.get("artifact_digest","")).startswith("sha256:"):
            raise ValueError(f"{key} artifact digest missing")
    ...
    if activation.get("fresh_empirical_opened") is not False:
        raise ValueError("fresh empirical data opened before prospective KT")
...
def main() -> None:
    ...
    if world_name not in worlds:
        raise ValueError("world outside prospective contract")
    world_index=worlds.index(world_name)

    blocks=execution["sharding"]["seed_blocks"]
    block_index=int(args.seed_block)
    if block_index<0 or block_index>=len(blocks):
        raise ValueError("seed block outside prospective execution profile")
    ...
    if not set(seeds).issubset(declared):
        raise ValueError("prospective seed block drift")
    ...
    manifest={
        ...
        "integration_artifact_digest":activation["integration_prerequisite"]["artifact_digest"],
        "outputs":{
            name:_sha256(outdir/name)
            for name in (
                "stage_p_states.csv","stage_t_states.csv",
                "odo_states.csv","full_system_gate.csv",
            )
        },
    }
