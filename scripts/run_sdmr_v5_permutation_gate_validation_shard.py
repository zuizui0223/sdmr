#!/usr/bin/env python3
"""Run one frozen SDMR v5 permutation-gate validation shard."""
...
def main() -> None:
    ...
    if config.get("program")!="sdmr-v5-permutation-gate-validation-v1":
        raise ValueError("wrong v5 permutation validation contract")
    if config.get("status")!="development_only_frozen_before_outcomes":
        raise ValueError("v5 validation contract not frozen")
    if activation.get("purpose")!="execute_sdmr_v5_permutation_gate_validation_v1":
        raise ValueError("wrong v5 validation activation")
    if activation.get("single_activation") is not True:
        raise ValueError("v5 validation activation must be single-use")
    if activation.get("config_blob_sha")!=_git_blob_sha(config_path):
        raise ValueError("v5 validation config blob SHA mismatch")
    ...
    if world not in worlds:
        raise ValueError("world outside frozen v5 validation contract")
