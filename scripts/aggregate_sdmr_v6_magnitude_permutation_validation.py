#!/usr/bin/env python3
"""Aggregate SDMR v6 magnitude-permutation validation evidence."""
...
def main() -> None:
    ...
    if len(manifests)!=int(args.expected_shards):
        raise ValueError(f"expected {args.expected_shards} v6 validation shards, found {len(manifests)}")
    for path in manifests:
        item=_load(path)
        if item.get("status")!="development_validation_evidence":
            raise ValueError("unexpected v6 validation shard status")
        if item.get("config_sha256")!=_sha256(config_path):
            raise ValueError("v6 validation config hash drift")
        if item.get("activation_sha256")!=_sha256(activation_path):
            raise ValueError("v6 validation activation hash drift")
    ...
    if len(summary_files)!=expected or len(fold_files)!=expected:
        raise ValueError("incomplete v6 validation outputs")
    ...
    if set(summary["world"].astype(str))!=expected_worlds:
        raise ValueError("v6 validation world denominator drift")
    if set(summary["seed"].astype(int))!=expected_seeds:
        raise ValueError("v6 validation seed denominator drift")
    if summary.duplicated(["world","seed"]).any():
        raise ValueError("duplicate v6 validation cells")
    if len(summary)!=len(expected_worlds)*len(expected_seeds):
        raise ValueError("unexpected v6 validation row count")
