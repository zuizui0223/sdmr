...
from sdmr.v2_6_continuation_v2_promotion import (
    apply_continuation_v2_promotion,
    load_continuation_v2_promotion_contract,
)

CONFIG = Path('configs/product_a_v2_6_continuation_v2_promotion_contract.json')
...
def test_contract_pins_exact_cont2_source_before_outcome():
    c = load_continuation_v2_promotion_contract(CONFIG)
    assert c['contract_frozen_before_continuation_v2_sealed_outcome'] is True
    assert c['new_postoutcome_scientific_thresholds'] is False
    src = c['independent_empirical_continuation_v2_source']
    assert src['implementation_sha'] == 'd560076793199b5486b7d2c678eeb4b4c551a81f'
    assert src['frozen_ref'] == 'frozen/product-a-v2-6-continuation-v2-d5600767'
    assert src['workflow_file'] == 'product-a-v2-6-empirical-presealed-continuation-v2.yml'
    assert src['artifact_name'].endswith('-continuation-v2')
    assert src['failed_predecessor_continuation_run_id'] == 32434610154
    assert src['failed_predecessor_pretruth_artifacts'] == 0
    assert src['failed_predecessor_sealed_environment_opened'] is False


def test_supported_sources_promote_product_a(tmp_path):
    kt, emp = _sources(tmp_path)
    result = apply_continuation_v2_promotion(
