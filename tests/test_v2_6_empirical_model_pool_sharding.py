...
def _write_shard(
    ...
    *,
    ...
    blank_selection_trace: bool = False,
) -> None:
    ...
    if blank_selection_trace:
        (root / "selection_trace.csv").write_text("\n", encoding="utf-8")
    else:
...
def test_merge_m_workers_accepts_blank_optional_selection_trace(tmp_path):
    shards = tmp_path / "shards"
    _write_shard(shards / "m0", M_NAMES[0])
    _write_shard(shards / "m1", M_NAMES[1])
    _write_shard(shards / "m2", M_NAMES[2], blank_selection_trace=True)
    out = tmp_path / "merged"
    result = merge_m_workers(worker_root=shards, output_dir=out)
    assert result["M_specs"] == list(M_NAMES)
    # The blank trace contributes zero optional rows; the nonblank M traces remain.
    trace = pd.read_csv(out / "selection_trace.csv")
    assert set(trace["M"]) == {M_NAMES[0], M_NAMES[1]}
    assert len(trace) == 2
    # Scientific evidence tables still retain all three M values.
    assert set(pd.read_csv(out / "base_fold_metrics.csv")["M"]) == set(M_NAMES)
