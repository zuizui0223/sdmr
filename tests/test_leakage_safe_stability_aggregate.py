...
def test_aggregate_leakage_safe_stability_parts(tmp_path: Path):
    ...
    for fraction in fractions:
        for seed in seeds:
            ...
            # Make one complete protocol dominate 11/15 runs so aggregation has
            # a non-trivial stability distribution.
            strategy = "predictive" if i < 11 else "vif"
            (root / "product_a_protocol_choice.txt").write_text(
                "winning_data_specification=buffer_300km\n"
            ...
            pd.DataFrame(
                {
                    "species": ["d", "e", "d", "e"],
                    "strategy": [strategy, strategy, "all", "all"],
                    "presence_rank": [0.7, 0.75, 0.65, 0.70],
                    "n_predictors": [2, 2, 4, 4],
                    "selected_by_discovery": [True, True, False, False],
                }
            ).to_csv(root / "protocol_validation_metrics.csv", index=False)
            pd.DataFrame(
                {
                    "species": ["d", "e"],
                    "winning_strategy": [strategy, strategy],
                    "comparator": ["all", "all"],
                    "delta_presence_rank": [0.05, 0.05],
                }
    ...
    assert deltas.loc[deltas["comparator"] == "all", "n_pairs"].iloc[0] == 30
