...
from sdmr.conditional_shared_knockout_process_challenge import (
    fit_conditional_shared_knockout_process_challenge,
)
from sdmr.model import ModelSpec


def _tables(seed=77, n=240):
...
def test_v8_exports_conditional_routes_and_preserves_retained_predictors():
    ...
    assert len(fit.conditional_route_summary) == 3
    assert len(fit.conditional_fold_evidence) == 12
    assert fit.intervention_audit["complete"].astype(bool).all()
    assert fit.intervention_audit["retained_predictors_unchanged"].astype(bool).all()
    assert set(fit.process_summary["process"]) == {"temperature", "water", "soil"}
    assert fit.selection_receipt != fit.v5_fit.selection_receipt


def test_v8_prediction_model_is_inherited_from_v5_not_retuned():
    p, b, pg, bg, registry = _tables(seed=92)
    fit = fit_conditional_shared_knockout_process_challenge(
        p,
        b,
        pg,
