...
CONTRACT = Path("configs/sdmr_v3_shallow3_positive_power_tail_v2.json")


def test_shallow3_power_tail_v2_changes_only_multiplier_extension():
    payload = json.loads(CONTRACT.read_text(encoding="utf-8"))
    assert payload["status"] == "development_only"
    assert payload["product_a_boundary"] == "closed_not_reopened"
    assert payload["development_predecessor"] == "sdmr-v3-shallow3-positive-power-v1"
    ...
    assert payload["sample_multipliers"] == [8]
    ...
    assert payload["power_v1_target"]["workflow_run"] == 35976809272
    assert payload["power_v1_target"]["completed_multipliers"] == [1,2,4]
